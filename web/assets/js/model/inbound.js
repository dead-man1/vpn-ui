...
TlsStreamSettings.Settings = class extends XrayCommonClass {
  constructor(fingerprint = UTLS_FINGERPRINT.UTLS_CHROME, echConfigList = "") {
    super();
    this.fingerprint = fingerprint;
    this.echConfigList = echConfigList;
  }
  static fromJson(json = {}) {
    return new TlsStreamSettings.Settings(json.fingerprint, json.echConfigList);
  }
  toJson() {
    return {
...
class RealityStreamSettings extends XrayCommonClass {
  constructor(
    ...
    if (!target && !serverNames) {
      const randomTarget =
        typeof getRandomRealityTarget !== "undefined"
...
class UdpMask extends XrayCommonClass {
  ...
  static fromJson(json = {}) {
    return new UdpMask(json.type || "salamander", json.settings || {});
  }
  ...
  toJson() {
    return {
      type: this.type,
      settings:
        this.settings && Object.keys(this.settings).length > 0
          ? this.settings
          : undefined,
    };
  }
...
class FinalMaskStreamSettings extends XrayCommonClass {
  constructor(udp = []) {
    super();
    this.udp = Array.isArray(udp)
      ? udp.map((u) => new UdpMask(u.type, u.settings))
      : [new UdpMask(udp.type, udp.settings)];
  }
...
class Inbound extends XrayCommonClass {
  ...
  canEnableVisionSeed() {
    ...
    return clients.some(
      (c) =>
        c?.flow === TLS_FLOW_CONTROL.VISION ||
        c?.flow === TLS_FLOW_CONTROL.VISION_UDP443,
    );
  }
...
Inbound.VLESSSettings = class extends Inbound.Settings {
  ...
  static fromJson(json = {}) {
    ...
    if (
      json.testseed &&
      Array.isArray(json.testseed) &&
      json.testseed.length >= 4
    ) {
  ...
  toJson() {
    ...
    const hasFlow =
      this.vlesses &&
      this.vlesses.some((vless) => vless.flow && vless.flow !== "");
...
Inbound.MixedSettings = class extends Inbound.Settings {
  constructor(
    protocol,
    auth = "password",
    accounts = [new Inbound.MixedSettings.SocksAccount()],
    udp = false,
    ip = "127.0.0.1",
  ) {
    super(protocol);
  ...
  static fromJson(json = {}) {
    ...
    if (json.auth === "password") {
      accounts = json.accounts.map((account) =>
        Inbound.MixedSettings.SocksAccount.fromJson(account),
      );
    }
